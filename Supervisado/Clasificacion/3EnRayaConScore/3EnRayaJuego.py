...
def jugarMaquina(turno):
    ...
    #Indicamos que es -1 como que no ha habido movimiento registrado
    movimiento=-1
    #Ponemos mejor a -1
    mejor=-1.0
    print("Muestras")
    ...
    #Se prueban las casillas del 0 al 8
    for i in range(9):
        #Si la posicion esta libre
        if tablero[i]==0:
            #Si no ha habido ningun movimiento, cogemos ese "por defecto"
            if movimiento==-1:
                movimiento=i
            #Generamos la muestra, con el estado del tablero, la posicion a la que movemos y el turno actual
            muestra=tablero + [i] + [turno]
            
            pred=classif.predict_proba([muestra])

            #Sumar aleatoriedad
            #pred[0][1]=pred[0][1]+random.uniform(0.0,0.05)
